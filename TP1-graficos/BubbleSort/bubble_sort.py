...
def test(size, sorting_type, data_type, print_array):
    ...
    comparisons, swaps = bubble_sort(array)
    end_time = time.time()
    
    print(f"Bubble Sort: {(end_time - start_time) * 1e6:.2f} us")
...
def run_tests():
    ...
    for data_type in data_types:
        for metric in ['tempos', 'comparacoes', 'trocas']:
            ...
            plt.title(f'Bubble Sort => {metric.capitalize()} por Entrada ({data_type.__name__})')
            plt.legend()
            plt.grid(True)
